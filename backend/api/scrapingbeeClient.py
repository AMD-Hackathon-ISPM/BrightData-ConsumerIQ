    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            while retries < max_retries:
                ...
                status_code = error.get('statusCode', 0)
...
def scrapeAmazonReviewPage(url: str, country_code: str = 'us', page: int = 1):
    ...
    extract_rules = {
        ...
        'averageRating': {'selector': '[data-hook=\'rating-out-of-text\']', 'output': 'text'},
        'totalReviews': {'selector': '[data-hook=\'total-review-count\']', 'output': 'text'},
    }
    ...
    for page in range(1, max_pages + 1):
        ...
        if error:
            if isBlockedScrapeError(error):
                return None, {
                    'status': 'blocked',
                    'sourceUrl': error.get('sourceUrl', url) if isinstance(error, dict) else url,
                    'message': 'Amazon redirected ScrapingBee to login/CAPTCHA before reviews were available',
                    'nextStep': 'Use a permitted data source fallback now, then switch this path to Bright Data Amazon reviews once credits are active.',
                }
            return None, error
        ...
        if page == 1:
            rating_summary = {
                ...
                'totalReviews': result.get('totalReviews'),
            }

    return {
        'reviews': all_reviews,
        'ratingSummary': rating_summary,
        'reviewCountScraped': len(all_reviews),
    }, None
